...
class DataValidation:
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info("replace 'na',' ?','?' with np.NAN")
            ...
            # like ' abc' --> 'abc'
            logging.info("replacing all np.NAN with mode of that column")
            column_object = base_df.select_dtypes(include = 'object').columns
            for i in column_object:
                base_df[i] = base_df[i].str.strip()
            base_df["workclass"] = base_df["workclass"].replace(np.NAN,base_df["workclass"].mode()[0])
            base_df["occupation"] = base_df["occupation"].replace(np.NAN,base_df["occupation"].mode()[0])
            base_df["country"] = base_df["country"].replace(np.NAN,base_df["country"].mode()[0])
